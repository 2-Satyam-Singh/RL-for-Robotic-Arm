...
analysis, is the gap we address.
...
\end{tikzpicture}}
\caption{Configuration-driven control flow of the framework.}
\label{fig:framework}
\end{figure*}
...
morphologies. Teacher training curves (moving-average reward, shaded $\pm$1 std) are shown in Fig.~\ref{fig:teacher}; lower-DoF arms converge faster.
...
\begin{subfigure}[b]{0.49\columnwidth}
  ...
  \caption{7-DoF Panda}
\end{subfigure}
\caption{PPO teacher training curves.}
\label{fig:teacher}
\end{figure}
...
The full implementation is available at \url{https://github.com/Satyam-Robotics/RL-for-Robotic-Arm/tree/v1.0-paper-submission}.
...
\end{subfigure}
\caption{Per-morphology closed-loop comparison.}
\label{fig:overlay}
\end{figure*}
...
\caption{Closed-loop task success across all morphologies.}
\label{fig:bars}
\end{figure*}
...
\caption{Teacher--student transfer curve on the 7-DoF Panda.}
\label{fig:transfer}
\end{figure}
...
separates the students. On the redundant 7-DoF arm---where the PPO
